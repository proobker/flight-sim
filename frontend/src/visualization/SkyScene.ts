...
export class SkyScene {
  ...
  /** Set once the authoritative elevation grid fetch settles (loaded or flat fallback). */
  ...
  /** True once the aircraft GLB has finished loading (or its fallback is chosen). */
  get modelReady(): boolean {
    return glbSettled;
  }

  /** True once the terrain grid request has settled (data fetched, or flat fallback). */
  get terrainLoaded(): boolean {
    return this.terrainSettled;
  }
